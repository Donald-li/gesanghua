...
class GenDonateCertificate
  class << self
    def create(record)
      ...
      # name |= '用户'
      render_white_text(source, 100, 460, "#{record.project_name}项目")
      render_white_text(source, 100, 477, "__________________________")
      ...
      content = "　　感谢您为支持格桑花西部助学#{record.project_name}项目的慷慨捐赠。谨代表受帮助的孩子向您表示最崇高敬意和衷心感谢！"
      render_content(source, content)

      render_text(source, 550, 1035, Date.today.year.to_s)
      render_text(source, 610, 1035, '年')
      render_text(source, 640, 1035, Date.today.month.to_s)
      render_text(source, 662, 1035, '月')
      render_text(source, 700, 1035, Date.today.day.to_s)
      render_text(source, 722, 1035, '日')
      ...
      rel_path = "/images/certificates/#{record.id}/#{record.id}"
