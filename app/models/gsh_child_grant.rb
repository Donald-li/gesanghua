...
#  gsh_child_id                  :bigint(8)                              # 关联孩子表id
#  project_season_apply_id       :bigint(8)                              # 关联申请表
...
class GshChildGrant < ApplicationRecord
  ...
  def self.gen_grant_record(child)
    ...
    if child.next_term? && apply_num > 0
      GshChildGrant.find_or_create_by(title: "#{year}.3 - #{year}.7 学期 #{level}.#{grade}", gsh_child: gsh_child, apply_child: child, apply: apply, amount: term_amount, school_id: child.school_id)
      apply_num -= 1
      grade += 1
    end
    ...
    if (apply_num > 0)
      apply_num.times do
        GshChildGrant.find_or_create_by(title: "#{year}.9 - #{year + 1}.7 学年 #{level}.#{grade}", gsh_child: gsh_child, apply_child: child, apply: apply, amount: year_amount, school_id: child.school_id)
        year += 1
        grade += 1
      end
    end
  ...
  def check_student_state
    if self.state == 'granted'
      child = self.apply_child
