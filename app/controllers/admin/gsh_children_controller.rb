class Admin::GshChildrenController < Admin::BaseController
  ...
  def index
    @search = ProjectSeasonApplyChild.sorted.ransack(params[:q])
    if @search.user_id
  ...
  def new
    @gsh_child = ProjectSeasonApplyChild.new
  end

  def create
    @gsh_child = ProjectSeasonApplyChild.new(gsh_child_params)
  ...
  private
  def set_gsh_child
    @gsh_child = ProjectSeasonApplyChild.find(params[:id])
  end
