class Admin::PairReportsController < Admin::BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @report.save
        format.html { redirect_to referer_or(admin_pair_reports_url), notice: '报告已增加。' }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @report.update(report_params)
        format.html { redirect_to referer_or(admin_pair_reports_url), notice: '报告已修改。' }
      else
  ...
  def destroy
    @report.destroy
    respond_to do |format|
      format.html { redirect_to referer_or(admin_pair_reports_url), notice: '报告已删除。' }
    end
  end
