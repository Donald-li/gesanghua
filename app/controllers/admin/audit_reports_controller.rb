...
class Admin::AuditReportsController < Admin::BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @audit_report.save
        ...
        format.html { redirect_to referer_or(admin_audit_reports_path), notice: '新增成功。' }
      else
  ...
  def update
    respond_to do |format|
      ...
      if @audit_report.update(audit_report_params)
        format.html { redirect_to referer_or(admin_audit_reports_path), notice: '修改成功。' }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to referer_or(admin_audit_reports_path), notice: '删除成功。' }
    end
  end

  def switch
    @audit_report.show? ? @audit_report.hidden! : @audit_report.show!
    redirect_to referer_or(admin_audit_reports_path), notice:  @audit_report.show? ? '报告已显示' : '报告已隐藏'
  end
