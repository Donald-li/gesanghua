class Admin::ProjectSeasonApplyCampStudentsController < Admin::BaseController
  ...
  def create
    @camp_student = ProjectSeasonApplyCampMember.new(camp_student_params.merge(camp: @apply_camp.camp, apply: @apply_camp.apply, school: @apply_camp.school, apply_camp: @apply_camp, kind: 'student', state: 'pass'))
  ...
  def excel_import
    respond_to do |format|
      result = ProjectSeasonApplyCampMember.read_excel(params[:apply_camp_excel_id], @apply_camp, 'student')
      if result[:status]
