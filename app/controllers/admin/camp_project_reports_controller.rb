class Admin::CampProjectReportsController < Admin::BaseController
  ...
  def create
    @report = ProjectReport.new(report_params.merge(project_id: Project.camp_project.id, user_id: current_user.id))
    @report.attach_images(params[:image_ids])
    respond_to do |format|
      if @report.save
        format.html { redirect_to referer_or(admin_camp_project_reports_url), notice: '报告已增加。' }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @report.update(report_params)
        format.html { redirect_to referer_or(admin_camp_project_reports_url), notice: '报告已修改。' }
      else
  ...
  def destroy
    @report.destroy
    respond_to do |format|
      format.html { redirect_to referer_or(admin_camp_project_reports_url), notice: '报告已删除。' }
    end
  end

  def switch
    @report.show? ? @report.hidden! : @report.show!
    redirect_to referer_or(admin_camp_project_reports_url), notice:  @report.show? ? '报告已展示' : '报告已隐藏'
  end
