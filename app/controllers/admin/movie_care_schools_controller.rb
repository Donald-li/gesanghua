class Admin::MovieCareSchoolsController < Admin::BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @project_apply.save
        ...
        format.html { redirect_to admin_movie_care_schools_path, notice: '新增成功。' }
      else
  ...
  def update
    respond_to do |format|
      if @project_apply.update(project_apply_params)
        @project_apply.attach_images(params[:image_ids])
        format.html { redirect_to admin_movie_care_schools_path, notice: '修改成功。' }
      else
        format.html { render :edit }
      end
    end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_movie_care_schools_path, notice: '删除成功。' }
    end
  end
