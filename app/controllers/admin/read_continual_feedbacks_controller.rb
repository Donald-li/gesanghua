class Admin::ReadContinualFeedbacksController < Admin::BaseController
  ...
  def create
    @continual = Continual.new(continual_params.merge(owner_type: 'ProjectSeasonApply', project_id: ProjectSeason.book_project_id, project_season_apply_id: continual_params[:owner_id], project_season_id: ProjectSeasonApply.find(continual_params[:owner_id]).season.id))
  ...
  def update
    respond_to do |format|
      if @continual.update(continual_params.merge(owner_type: 'ProjectSeasonApply', project_id: ProjectSeason.book_project_id, project_season_apply_id: continual_params[:owner_id], project_season_id: ProjectSeasonApply.find(continual_params[:owner_id]).season.id))
  ...
  def destroy
    @continual.destroy
    respond_to do |format|
      format.html { redirect_to admin_read_continual_feedbacks_path(@project), notice: '删除成功。' }
    end
  end

  def recommend
    @continual.recommend? ? @continual.normal! : @continual.recommend!
    redirect_to admin_read_continual_feedbacks_path(@project), notice:  @continual.recommend? ? '已推荐反馈' : '已取消推荐反馈'
  end
  ...
  private
    ...
    def set_project_apply
      @apply = ProjectSeasonApply.find(params[:book_project_id])
    end
end
