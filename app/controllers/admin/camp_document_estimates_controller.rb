class Admin::CampDocumentEstimatesController < Admin::CampDocumentBaseController
  ...
  def index
    @search = CampDocumentEstimate.in_apply(@current_camp).sorted.ransack(params[:q])
    scope = @search.result
  ...
  def create
    @camp_document_estimate = CampDocumentEstimate.new(camp_document_estimate_params.merge(user: current_user, apply: @current_apply))
