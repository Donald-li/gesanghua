class Admin::MovieContinualFeedbacksController < Admin::BaseController
  ...
  def create
    @continual = Continual.new(continual_params.merge(owner_type: 'ProjectSeasonApply', project_id: ProjectSeason.movie_project_id, project_season_apply_id: continual_params[:owner_id], project_season_id: ProjectSeasonApply.find(continual_params[:owner_id]).season.id))
    respond_to do |format|
      if @continual.save
        format.html { redirect_to admin_movie_continual_feedbacks_path(@project), notice: '新增成功。' }
      else
  ...
  def update
    respond_to do |format|
      if @continual.update(continual_params.merge(owner_type: 'ProjectSeasonApply', project_id: ProjectSeason.movie_project_id, project_season_apply_id: continual_params[:owner_id], project_season_id: ProjectSeasonApply.find(continual_params[:owner_id]).season.id))
  ...
  private
    ...
    def set_project_apply
      @apply = ProjectSeasonApply.find(params[:movie_project_id])
    end
end
