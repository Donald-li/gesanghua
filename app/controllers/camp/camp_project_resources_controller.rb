class Camp::CampProjectResourcesController < Camp::BaseController
  ...
  def create
    @camp_project_resource = CampProjectResource.new(camp_project_resource_params.merge(user: current_user, camp: current_user.camp))
    respond_to do |format|
      if @camp_project_resource.save
        format.html { redirect_to camp_camp_project_resources_url, notice: '新增成功' }
      else
  ...
  def update
    respond_to do |format|
      if @camp_project_resource.update(camp_project_resource_params)
        format.html { redirect_to camp_camp_project_resources_url, notice: '修改成功' }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to camp_camp_project_resources_url, notice: '删除成功' }
    end
  end
