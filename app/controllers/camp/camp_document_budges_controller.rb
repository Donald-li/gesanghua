class Camp::CampDocumentBudgesController < Camp::CampDocumentBaseController
  ...
  def create
    ...
    respond_to do |format|
      if @camp_document_finance.save
        format.html { redirect_to camp_camp_document_budges_url, notice: '新增成功' }
      else
  ...
  def update
    respond_to do |format|
      if @camp_document_finance.update(camp_document_finance_params)
        format.html { redirect_to camp_camp_document_budges_url, notice: '修改成功' }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to camp_camp_document_budges_url, notice: '删除成功' }
    end
  end
