class Api::V1::CooperationRegularFeedbacksController < Api::V1::BaseController
  ...
  def feedback_list
    feedbacks =  @project.continual_feedbacks.page(params[:page]).per(params[:per])
    api_success(data: {feedbacks: feedbacks.map{|f| f.detail_builder}, pagination: json_pagination(feedbacks)})
  end

  def qrcode
    url = "http://#{Settings.app_host}/cooperation/link-to-visit?type=regular_feedback&project_id=#{@project.id}"
