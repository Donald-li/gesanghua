class Api::V1::MainsController < Api::V1::BaseController
  ...
  def contribute
    ...
    api_success(data: @projects.map{|project| project.options_builder})
  end
  ...
  def settlement
    ...
    if project.present?
      record = DonateRecord.donate_project(current_user, amount, project, promoter)
    else
  ...
  def month_contribute
    ...
    project = Project.find_project(params[:project][:value])
    if project.present?
      record = MonthDonate.month_donate_project(current_user, amount, project, plan_period)
    else
      record = MonthDonate.month_donate_gsh(current_user, amount, plan_period)
    end
