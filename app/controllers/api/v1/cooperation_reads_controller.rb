  ...
  def index
    ...
    if user.teacher.present?
      if user.has_role?(:headmaster)
        applies = user.teacher.school.project_season_applies.includes(:project, :season).where(project_id: @read.id).sorted.page(params[:page])
        api_success(data: {applies: applies.map { |r| r.read_applies_builder }, pagination: json_pagination(applies)})
      elsif user.has_role?(:teacher)
        applies = user.teacher.project_season_applies.includes(:project, :season).where(project_id: @read.id).sorted.page(params[:page])
        api_success(data: {applies: applies.map { |r| r.read_applies_builder }, pagination: json_pagination(applies)})
      else
