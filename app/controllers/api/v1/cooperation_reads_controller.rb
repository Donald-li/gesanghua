class Api::V1::CooperationReadsController < Api::V1::BaseController
  ...
  def update
    ...
    if @apply.update(bookshelf_ids: class_ids, project_season_id: season_id, class_number: class_number, student_number: student_number, contact_name: contact_name, contact_phone: contact_phone, province: province, city: city, district: district, address: address)
      @apply.attach_images(params[:images])
      api_success(data: {result: true})
    else
      api_success(data: {result: false})
